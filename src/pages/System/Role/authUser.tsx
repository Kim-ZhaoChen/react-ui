...
const cancelAuthUserAll = async (roleId: string, selectedRows: API.System.User[]) => {
	const hide = message.loading('正在Cancel授权');
	...
	try {
		...
		hide();
		if (resp.code === 200) {
			message.success('Cancel授权成功，即将刷新');
		} else {
			...
			render: (_, record) => [
				<Button
					...
					onClick={async () => {
						Modal.confirm({
							title: 'Delete',
							content: 'Confirm要Cancel该用户' + record.userName + '"角色授权吗？',
							okText: 'Confirm',
							cancelText: 'Cancel',
							onOk: async () => {
			...
			<div style={{ width: '100%', float: 'right' }}>
				<ProTable<API.System.User>
					...
					toolBarRender={() => [
						...
						>
							<DeleteOutlined />
							<FormattedMessage id="system.role.auth.cancelAll" defaultMessage="批量Cancel授权" />
						</Button>,
						<Button
							type="primary"
							key="back"
